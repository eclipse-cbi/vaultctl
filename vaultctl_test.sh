...
# Test cases for vaultctl select (profile management - no vault connection required)
test_vaultctl_select() {
    local result
    local config_file="$VAULT_CONFIG_FILE"
    ...
    # Start from a clean config (default profile only)
    rm -f "$config_file"
    echo "VAULT_ADDR=https://secretsmanager.eclipse.org" > "$config_file"
    echo "VAULT_USERNAME=john" >> "$config_file"
    chmod 600 "$config_file"

    ######## Show current profile (default) ########################################
    result=$("$VAULTCTL" select 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"Profile : default"* ]]; then
        echo -e "INFO: vaultctl select shows default profile \u2705"
    else
        echo -e "ERROR: vaultctl select should show default profile \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi
    ...
    result=$("$VAULTCTL" select create staging --addr https://vault.staging.example.com 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"Profile 'staging' created"* ]]; then
        echo -e "INFO: vaultctl select create staging \u2705"
    else
        echo -e "ERROR: vaultctl select create staging failed \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi

    # Verify [staging] section was written with correct VAULT_ADDR
    if grep -q "^\[staging\]" "$config_file" && grep -A5 "^\[staging\]" "$config_file" | grep -q "VAULT_ADDR=https://vault.staging.example.com"; then
        echo -e "INFO: staging section written to config \u2705"
    else
        echo -e "ERROR: staging section missing or incorrect in config \u274c"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi

    ######## Duplicate create should fail ##########################################
    result=$("$VAULTCTL" select create staging 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"already exists"* ]]; then
        echo -e "INFO: vaultctl select create duplicate rejected \u2705"
    else
        echo -e "ERROR: vaultctl select create duplicate should be rejected \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi
    ...
    result=$("$VAULTCTL" select create default 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"Cannot create"* ]]; then
        echo -e "INFO: vaultctl select create default rejected \u2705"
    else
        echo -e "ERROR: vaultctl select create default should be rejected \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi
    ...
    result=$("$VAULTCTL" select list 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"default"* && "$result" == *"staging"* ]]; then
        echo -e "INFO: vaultctl select list shows both profiles \u2705"
    else
        echo -e "ERROR: vaultctl select list missing profiles \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi

    # Default should be marked active (*)
    if echo "$result" | grep -q "\* *default"; then
        echo -e "INFO: vaultctl select list marks default as active \u2705"
    else
        echo -e "ERROR: vaultctl select list should mark default as active \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi
    ...
    result=$("$VAULTCTL" select staging 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"Switched to profile: staging"* ]]; then
        echo -e "INFO: vaultctl select staging \u2705"
    else
        echo -e "ERROR: vaultctl select staging failed \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi

    # Verify CURRENT_PROFILE written before first [section] header
    local cp_line sec_line
    cp_line=$(grep -n "^CURRENT_PROFILE=" "$config_file" | head -1 | cut -d: -f1)
    sec_line=$(grep -n "^\[" "$config_file" | head -1 | cut -d: -f1)
    if [[ -n "$cp_line" && -n "$sec_line" && "$cp_line" -lt "$sec_line" ]]; then
        echo -e "INFO: CURRENT_PROFILE is in flat (default) section \u2705"
    else
        echo -e "ERROR: CURRENT_PROFILE not in flat section (cp=$cp_line sec=$sec_line) \u274c"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi

    ######## Switch to non-existent profile should fail ############################
    result=$("$VAULTCTL" select nonexistent 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"does not exist"* ]]; then
        echo -e "INFO: vaultctl select nonexistent rejected \u2705"
    else
        echo -e "ERROR: vaultctl select nonexistent should be rejected \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi
    ...
    result=$("$VAULTCTL" select default 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"Switched to profile: default"* ]]; then
        echo -e "INFO: vaultctl select default \u2705"
    else
        echo -e "ERROR: vaultctl select default failed \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi
    ...
    result=$("$VAULTCTL" select delete default 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"Cannot delete"* ]]; then
        echo -e "INFO: vaultctl select delete default rejected \u2705"
    else
        echo -e "ERROR: vaultctl select delete default should be rejected \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi
    ...
    "$VAULTCTL" select staging > /dev/null 2>&1 || true
    result=$("$VAULTCTL" select delete staging 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"Cannot delete the active profile"* ]]; then
        echo -e "INFO: vaultctl select delete active profile rejected \u2705"
    else
        echo -e "ERROR: vaultctl select delete active profile should be rejected \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi
    ...
    "$VAULTCTL" select default > /dev/null 2>&1 || true
    result=$("$VAULTCTL" select delete staging 2>&1 | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$result" == *"Profile 'staging' deleted"* ]]; then
        echo -e "INFO: vaultctl select delete staging \u2705"
    else
        echo -e "ERROR: vaultctl select delete staging failed \u274c"
        echo "Result: $result"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi

    # Verify [staging] section is gone
    if ! grep -q "^\[staging\]" "$config_file"; then
        echo -e "INFO: staging section removed from config \u2705"
    else
        echo -e "ERROR: staging section still present after delete \u274c"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi

    ######## Profile config isolation: config written per profile ##################
    # Create fresh staging profile
    "$VAULTCTL" select create staging --addr https://vault.staging.example.com > /dev/null 2>&1
    "$VAULTCTL" select staging > /dev/null 2>&1 || true

    # Write a config key while on staging - should land in [staging] section
    "$VAULTCTL" config VAULT_MOUNT=staging-mount > /dev/null 2>&1 || true

    # Verify it's in [staging] section, not in default section
    local in_staging=false
    local cur_section="default"
    while IFS= read -r line; do
        if [[ "$line" =~ ^\[([A-Za-z0-9_-]+)\]$ ]]; then
            cur_section="${BASH_REMATCH[1]}"
            continue
        fi
        if [[ "$cur_section" == "staging" && "$line" == "VAULT_MOUNT=staging-mount" ]]; then
            in_staging=true
        fi
    done < "$config_file"

    if [[ "$in_staging" == "true" ]]; then
        echo -e "INFO: config key written to correct profile section \u2705"
    else
        echo -e "ERROR: config key not found in staging section \u274c"
        [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file"
        exit 1
    fi

    # Switch back to default and verify VAULT_MOUNT is not polluted
    "$VAULTCTL" select default > /dev/null 2>&1 || true
    local default_mount
    default_mount=$("$VAULTCTL" config 2>&1 | grep "VAULT_MOUNT" | grep -v "staging-mount" | sed 's/\x1b\[[0-9;]*m//g' || true)
    if [[ "$default_mount" != *"staging-mount"* ]]; then
    ...
    # Cleanup
    [[ -n "$backup_file" ]] && mv "$backup_file" "$config_file" || rm -f "$config_file"
}
...
test_vaultctl_write_default_mount
echo -e "\nTests for vaultctl select (profile management)...\n"
test_vaultctl_select
